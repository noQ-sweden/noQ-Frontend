    ...
    <div className="w-full">
      {accommodation.map((request) => (
        ...
        <div className="flex flex-col items-center md:items-end gap-2 w-full">
            <Link to={`/accommodations/${request.host.id}`} className="w-full md:w-auto">
              <button className="
            ...
            <p className="text-sm font-semibold">
              <Link to={`/accommodations/${request.host.id}`} className="flex items-center justify-center md:justify-start text-[#4CAA4A]">
                Mer information <FaChevronRight className="ml-2 text-gray-500"/>
              </Link>
            </p>
          </div>
        </div>
