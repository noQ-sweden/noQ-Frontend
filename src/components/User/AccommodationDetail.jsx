          ...
          <div>
            ...
            <div
              ...
              <div className="grid md:grid-cols-3">
                {host ? (
                  <>
                    <img src={shelter} alt={host.name}className="rounded-lg w-52 col-span-1"/>
                    <div className=" md:col-span-2 ">
                      <p className="text-base font-bold grid justify-items-start">{host.name}</p>
                      <p className="text-sm grid justify-items-start">{host.street}</p>
                      <p className="text-sm grid justify-items-start">{host.city}</p>
                    </div>
                  </>
                ) : (
              ...
              <div className="grid gap-4">
                ...
                {products ? (
                  <>
                    ...
                    <AccommodationAmenitiesDisplay title={"Rum"} products={products.filter(item => item.type=='room' || item.type=='woman-only')}/>
                    <AccommodationAmenitiesDisplay title={"Hygien"} products={products.filter(item => item.type=='hygieneproducts')}/>
                    <AccommodationAmenitiesDisplay title={"Mat"} products={products.filter(item => item.type=='breakfast' || item.type=='dinner')}/>
                    <AccommodationAmenitiesDisplay title={"Övrigt"} products={products.filter(item => item.type=='storage')}/>
                  </>
                ) : (
                    <p className="text-base font-bold grid justify-items-start">Boställen har inga produkter.</p>
                )}
                </div>
